...
def format_chat_template(input_text):
    ...
    parsed = grammar.parseString(input_text)

    # Initialize variables to keep track of user and assistant segments
    segments = []

    # Helper function to convert ParseResults or SavedTextNode to a string
    def to_str(segment):
        if isinstance(segment, pp.ParseResults):
            return ''.join(map(to_str, segment))
        elif isinstance(segment, SavedTextNode):
            return segment.text
        else:
            return str(segment)

    # To track whether we've encountered a {{gen ... }} command
    encountered_gen_command = False

    # Iterate through parsed segments
    for segment in parsed.program:
        if isinstance(segment, pp.ParseResults):
            segments.append('{{#user}}{}{{/user}}'.format(to_str(segment)))
        else:
            segments.append('{{#assistant}}{}{{/assistant}}'.format(to_str(segment)))
            encountered_gen_command=True

    # If no {{gen ... }} command was encountered, add {{gen 'write' }}
    if not encountered_gen_command:
        segments.append('{{#assistant}}{{gen \'write\' }}{{/assistant}}')

    # Construct the formatted chat template
    formatted_template = ''.join(segments)

    return formatted_template


# Testcases:
input_text1 = "Tweak this proverb to apply to model instructions instead.{{gen 'rewrite'}} Where there is no guidance"
input_text2 = "Tweak this proverb to apply to model instructions instead. Where there is no guidance{{gen 'rewrite'}} "
input_text3 = "{{gen 'rewrite'}} Tweak this proverb to apply to model instructions instead. Where there is no guidance"
input_text4 = "Tweak this proverb to apply to model instructions instead. Where there is no guidance"
formatted_output1 = format_chat_template(input_text1)
formatted_output2 = format_chat_template(input_text2)
formatted_output3= format_chat_template(input_text3)
formatted_output4= format_chat_template(input_text4)
